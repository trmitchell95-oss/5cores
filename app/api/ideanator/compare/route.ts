...
function extractPayload(row: SavedReportRow): VersionPayload {
  ...
  return {
    title: cleanText(row.title) || getStringField(ideanator, "ideaName") || "Untitled Little Bastard",
...
function fallbackComparison(previous: VersionPayload, current: VersionPayload): CompareResult {
  return {
    ...
    verdictMovement: `${previous.verdict} → ${current.verdict}`,
    clarityDelta:
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        error: "The comparison lift jammed.",
