...
TRANSPARENCY PROMPT PREVIEW:
---
${actualPrompt || "No prompt preview supplied."}
---
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const actualPrompt = cleanText(body.actualPrompt);
    ...
    inputChars =
      fog.length +
      actualPrompt.length +
      JSON.stringify(blueprint).length;
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "That rig is too heavy for beta mode. Shorten the fog or blueprint and try again.",
        },
        { status: 413 }
      );
    }
