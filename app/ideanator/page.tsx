...
## Why
${readiness.why.length ? readiness.why.map((item) => `- ${item}`).join("\n") : "- No reasons supplied."}

## Biggest Risk
${readiness.biggestRisk}

## Fix Next
${readiness.fixNext.length ? readiness.fixNext.map((item) => `- ${item}`).join("\n") : "- No fixes supplied."}

## Reusable
${readiness.reusable ? "Yes" : "Not yet"}
...
IDEA KIND:
${getIdeaReportString(ideanator, "ideaKind") || "Not specified"}

ORIGINAL NEED:
${getIdeaReportString(ideanator, "primaryNeed") || "Not specified"}

VERDICT:
${getIdeaReportString(ideanator, "verdict") || "Not specified"}

ORIGINAL USER IDEA / FOG:
---
${submittedText || "No original submitted idea text was found in the saved report."}
---

IDEA CHECK DIAGNOSTIC:

Spark:
${getIdeaReportString(ideanator, "spark") || "Not saved."}

Plain-English Version:
${getIdeaReportString(ideanator, "plainEnglishVersion") || "Not saved."}

Strongest Use Case:
${getIdeaReportString(ideanator, "strongestUseCase") || "Not saved."}

Weak Spots:
${getIdeaReportString(ideanator, "weakSpots") || "Not saved."}

Audience:
${getIdeaReportString(ideanator, "audience") || "Not saved."}

Money / Value Path:
${getIdeaReportString(ideanator, "moneyValuePath") || "Not saved."}

Avoidance:
${getIdeaReportString(ideanator, "avoidance") || "Not saved."}

Next Three Moves:
${moves.length ? moves.map((move, index) => `${index + 1}. ${move}`).join("\n") : "No saved next moves."}

TASK FOR RIG:
Turn this saved Idea Check report into a reusable thinking rig. The rig should help the user keep developing, pitching, testing, refining, or explaining this idea without losing the useful diagnostic insight.`;
}
